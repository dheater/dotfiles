...
# Carl - Auggie with personal context, critical persona, and JIRA MCP pre-loaded
carl() {
    ...
    echo "   JIRA MCP enabled for ticket management"
    echo ""
    ...
    if [[ -n "$personal_context" && -n "$work_patterns" ]]; then
        auggie --mcp-config $HOME/.augment/mcp-atlassian.json "$context_prompt
